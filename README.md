...
LingoTrace 是一个完全构建在 Obsidian 之上的、高度定制化和自动化的**外语学习工作流引擎**。
...
本项目包含五个高度内聚的 Agent Skills（详见 `agent-skills/` 目录），覆盖了外语学习”输入 -> 内化 -> 复习 -> 输出”的全生命周期：

1. **听力脚本生成器 (`listening-script-generator`)**：处理特定听力材料，生成带有重音标注的全文脚本，或基于切片生成逐句影子跟读（Shadowing）学习包，并强制提炼”常用句”。
2. **来源笔记生成器 (`source-note-generator`)**：处理广泛的音视频输入（YouTube、播客等），通过大模型自由萃取语法、词汇、表达，并确保完整的出处追溯（Provenance）。
3. **复习材料维护器 (`review-material-maintainer`)**：独创的双层词汇架构（Focus 焦点复习区 / Base 基础长时图谱），严格防重防漏地自动化维护用户的词汇、语法和错题关联网络。
4. **生存口语卡生成器 (`survival-speaking-card-generator`)**：将被动输入的”常用句”升维成基于特定真实生活场景（Scene）的口语输出卡片，并绑定发音切片。
5. **次日复习更新引擎 (`next-day-review-updater`)**：Obsidian 本地内建的间隔重复系统（SRS），负责时间衰减推算与卡片状态流转，处理词汇在 180 天后的”下沉归档”。
...
- `agent-skills/`：维持系统运转的各类 Agent 本地技能声明（`SKILL.md`）和底层自动化执行脚本。
